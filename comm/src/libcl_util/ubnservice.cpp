...
UBN_SERVER_HANDLE ubn_server_open(unsigned short port,const char* ubnname,bool bthreadroot)
{
	...
	string logpath = cl_util::get_module_dir()+"ubnserver.log";
	cl_util::write_tlog(logpath.c_str(),1024,"#ubn server open( bind port:%d )... \n",port);
...
int ubn_find_server(unsigned short port,const char* ubnname,ubn_server_t* us,unsigned int timeo/*=3000*/) //count 指定最多查找个数，返回实际查找个数
{
	...
	while(1)
	{
		if(sendto(fd,req_name,(int)strlen(req_name),0,(const sockaddr*)&addr,sizeof(addr))>0)
		{
			...
			if(us->count >= count || us->count>= UBN_MAX_FIND_NUM ||  begtick + timeo < GetTickCount())
				break;
...
int ubn_find_server_print(unsigned short port,const char* ubnname,unsigned int count,unsigned int timeo/*=3000*/)
{
	printf("# ubn find unbserver(%d,%s,%d): \n",port,ubnname,count);
	ubn_server_t us;
	us.count = count;
	in_addr ia;
	int n = ubn_find_server(port,ubnname,&us,timeo);
	for(int i=0;i<n;++i)
	{
		ia.s_addr = htonl(us.server[i].ip);
		printf("# $: %d => %s - %s \n",i+1,inet_ntoa(ia),us.server[i].name);
	}
	printf("# ubn find unbserver end \n");
	return 0;
}
