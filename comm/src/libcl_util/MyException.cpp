...
namespace MyException
{
	...
	int exception_callbak_default(MyExceptionInfo *p);
	...
	int exception_callbak_default(MyExceptionInfo *p)
	{
		char path[512];
		char app_path[512];
		::GetModuleFileNameA(NULL,app_path,512);
		...
		::GetLocalTime(&t);
		char msg[14000];
		sprintf(msg,"[%d-%d-%d %02d:%02d:%02d] [exception]:code=0x%08x,addr=0x%08llx \r\n[err_msg]:%s \r\n[call stack]:\r\n%s"
			,t.wYear,t.wMonth,t.wDay,t.wHour,t.wMinute,t.wSecond
			,p->code,(long long)p->addr,p->szerr_msg,p->szcall_stack);

		try
		{
			sprintf(path,"%s\\EXCEP.LOG",app_path);
	...
	#ifdef _DEBUG
		MessageBoxA(0,msg,"exe exception",MB_OK);
	#endif
		return 0;
	}
	int on_exception(EXCEPTION_POINTERS *pEP)
	{
		...
		//
		{
			// 先搞到模块名字
			MEMORY_BASIC_INFORMATION mem;
			VirtualQuery ((PVOID)(long long)pCon->Eip, &mem, sizeof (MEMORY_BASIC_INFORMATION));
			GetModuleFileNameA ((HMODULE)mem.AllocationBase, sztmp, sizeof (sztmp));
