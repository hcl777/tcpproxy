...
string& cl_inifile::string_trim(string& str,char c/*=' '*/)
{
...
string& cl_inifile::string_trim_endline(string& str)
{
	int pos1=0,pos2=(int)str.length()-1;
...
int cl_inifile::parse(const char * content )
{
	assert(!m_bopen);
	if(m_bopen)
		close();
...
bool cl_inifile::has_key(const char* section_name,const char* key_name )
{
	assert(m_bopen);
...
int cl_inifile::open(const char* path)
{
	assert(!m_bopen);
	if(m_bopen)
		close();
...
int cl_inifile::add_section(const char* section_name,const ini_key_t& key)
{
...
int cl_inifile::write_string(const char* section_name,const char* key_name,const char* inval)
{
	assert(m_bopen);
...
int cl_inifile::write_int(const char* section_name,const char* key_name,int inval)
{
	assert(m_bopen);
	m_bchanged = true;
...
char* cl_inifile::read_string(const char* section_name,const char* key_name,const char* default_val,char *outbuf,unsigned int outbuflen)
{
	assert(m_bopen);
...
int cl_inifile::read_int(const char* section_name,const char* key_name,int default_val)
{
	assert(m_bopen);
