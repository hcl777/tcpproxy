...
void cl_xml_test()
{
	...
	//cl_xmlNode *node = xml.find_first_node(NULL);
...
void cl_xmlAtrri::clear()
{
	...
	{
		n = ptr->next;
		CLXML_SAFEDELETE_ARRAY(ptr->name);
		CLXML_SAFEDELETE_ARRAY(ptr->val);
		delete ptr;
		ptr = n;
	}
	head.next = NULL;
}
int cl_xmlAtrri::load_string(const char* szAttri)
{
	clear();
	if(NULL==szAttri)
		return 0;
...
const char* cl_xmlAtrri::get_attri(const char* name)
{
	ANode_t *ptr;
	assert(name);
	ptr = head.next;
...
int cl_xmlAtrri::set_attri(const char* name,const char* val)
{
...
bool cl_xmlAtrri::check_attri(const char* name,const char* val)
{
	const char* a = get_attri(name);
	if(a && 0==strcmp(a,val))
		return true;
	return false;
}
int cl_xmlAtrri::size()
{
	int s = 4;
	ANode_t *ptr = head.next;
	while(ptr)
...
int cl_xmlAtrri::to_string(char* buf,int buflen)
{
	if(buflen<size())
		return -1;
	ANode_t *ptr = head.next;
...
bool cl_xml::string_english_only(const char* str)
{
...
int cl_xml::load_file(const char* path)
{
	clear();
	char *szXML = NULL;
	int size = 0;
...
int cl_xml::load_string(const char* szXML)
{
...
int cl_xml::set_xml_head(const char* head)
{
	if(!head)
		return -1;
...
cl_xmlNode *cl_xml::find_first_node(const char* tagPath,cl_xmlNode* start/*=NULL*/)
{
...
cl_xmlNode *cl_xml::find_first_node_attri(const char* tagPath,const char* attri_name,const char* attri_val,cl_xmlNode* start/*=NULL*/)
{
	cl_xmlNode *n = find_first_node(tagPath,start);
	const char *p = strrchr(tagPath,'/');
	p = p?(p+1):tagPath;
	while(n)
	{
		if(0==strcmp(n->get_tag(),p) && n->attri.check_attri(attri_name,attri_val))
			return n;
		n = n->next();
	}
	return NULL;
}
char* cl_xml::find_first_node_data(cl_xmlNode* start,const char* tagPath,const char* val_default,char buf[],int bufsize)
{
	//不管有无节点,只要空值就使用默认
	cl_xmlNode *n = find_first_node(tagPath,start);
	const char* d = NULL;
	if(n) d = n->get_data();
	if(n && d && strlen(d)>0 && bufsize > (int)strlen(d))
		strcpy(buf,d);
	else
		strcpy(buf,val_default);
	return buf;
}
const char* cl_xml::get_node_data(const char* tagPath,cl_xmlNode* start/*=NULL*/)
{
	cl_xmlNode* node = find_first_node(tagPath,start);
	if(node)
		return node->get_data();
	return NULL;
}
cl_xmlNode *cl_xml::find_node(const char* tag,int tagLen,const char* childTag,cl_xmlNode *node)
{
	if(0==tagLen)
		return NULL;
	cl_xmlNode *p = NULL;
...
int cl_xml::delete_node(cl_xmlNode *node)
{
	if(NULL==node)
		return -1;
	if(node->pparent)
...
//**************************
int cl_xml::load_next(const char* szMin,const char* szMax,cl_xmlNode* head)
{
	...
	if(p2>p1) p2=NULL;
	bool isEmpty = false;
	cl_xmlNode *node = new cl_xmlNode();
...
int cl_xml::load_child(const char* szMin,const char* szMax,cl_xmlNode* head)
{
...
const char* cl_xml::find_tag_end(const char* tag,const char* szMin,const char* szMax)
{
	const char *p1,*p2,*p3;
...
int cl_xml::save_file(const char* path)
{
	int isize = size();
	if(!isize)
		return -1;
...
int cl_xml::size_count(cl_xmlNode* head,int step)
{
	if(!head)
		return 0;
	int size = 0;
...
int cl_xml::to_string(char* szXML,int/* maxSize*/)
{
	if(!m_head.pnext)
		return -1;
	szXML[0] = '\0';
...
int cl_xml::format_string(char* szXML,cl_xmlNode* head,int step)
{
	if(!head)
		return 0;
	strcat(szXML,"\r\n");
...
cl_xmlNode *cl_xml::add_node(const char* path,const char* data/*=NULL*/)
{
	...
	if(0==strlen(p1+1))
		return NULL;
	p2 = path;
	cl_xmlNode *pre = &m_head;
	cl_xmlNode *parent = NULL;
	cl_xmlNode *node = pre->pnext;
	while(p2)
	{
		p3 = strchr(p2,'/');
		if(!p3 || p3>p1)
...
cl_xmlNode *cl_xml::add_child(cl_xmlNode *node,const char* tag,const char* data/*=NULL*/)
{
	if(!node||!tag||0==strlen(tag))
		return NULL;
	cl_xmlNode *child = new cl_xmlNode();
	if(!child)
		return NULL;
...
void cl_xml::delete_node_root(cl_xmlNode* head,bool delnext)
{
	if(head)
	{
