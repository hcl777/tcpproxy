...
int cl_BFile64::resize(ssize64_t size)
{
	if(_file.is_open())
	{
		if(size>0)
...
//int cl_BFile64::eif_get_node(cl_file64& file,ssize64_t bpos,RDBEIFNode_t& inf)
//{
//	char buf[256];
//	char buf_end[16];
...
//}
//
//int cl_BFile64::eif_copy_file(cl_file64& from,ssize64_t from_pos,cl_file64& to,ssize64_t to_pos,size64_t size)
//{
...
cl_RDBFile64Simple::cl_RDBFile64Simple(void)
:m_head_size(0)
,m_file_size(0)
,m_pos(0)
{
...
int cl_RDBFile64Simple::open(const char* path,int mode)
{
...
ssize64_t cl_RDBFile64Simple::seek(ssize64_t distance,int smode)
{
	if(SEEK_SET==smode)
		m_pos = distance;
...
int cl_RDBFile64Simple::read(char *buf,int len)
{
	if(!_file.is_open())
		return 0;
...
int cl_RDBFile64Simple::normal_to_rdbs_file(const char* path,const char* to_path)
{
	cl_file64 file;
...
int cl_RDBFile64Simple::rdbs_to_normal_file(const char* path,const char* to_path)
{
	cl_file64 file;
	if(0!=file.open(path,F64_RDWR))
		return -1;
