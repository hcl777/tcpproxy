...
public:
	...
	//
	////扩展信息文件相关接口--Extended information file
	//static int eif_get_node(cl_file64& file,ssize64_t bpos,RDBEIFNode_t& inf);
	//static int eif_copy_file(cl_file64& from,ssize64_t from_pos,cl_file64& to,ssize64_t to_pos,size64_t size);
protected:
	cl_file64 _file;
};
...
class cl_RDBFile64Simple : public cl_BFile64
{
public:
	cl_RDBFile64Simple(void);
	~cl_RDBFile64Simple(void);
public:
