...
int cl_file32::open(const char *_Filename,int _Mode)
{
	if(m_fp)
		return -1;
...
bool cl_file32::is_open() const 
{
	return (m_fp!=NULL);
}
void cl_file32::close() 
{
	if(m_fp)
	{
		fclose(m_fp);
...
int cl_file32::seek(int _Offset,int _Origin) 
{
	if(!m_fp)
	{
		assert(0);
...
int cl_file32::write(const char *buf,int len) 
{
	if(!m_fp)
	{
		assert(0);
...
int cl_file32::read(char *buf,int len)
{
	if(!m_fp)
	{
		assert(0);
...
char* cl_file32::getline(char *buf,int maxlen)
{
	if(!m_fp)
	{
		assert(0);
...
bool cl_file32::write_n(const char *buf,int len) 
{
	if(!m_fp)
	{
		assert(0);
...
bool cl_file32::read_n(char *buf,int len)
{
	if(!m_fp)
	{
		assert(0);
...
int cl_file32::remove_file(const char* path)
{
	return unlink(path);
}
int cl_file32::rename_file(const char* from,const char* to)
{
	return rename(from,to);
}
//*******************************************************************
#define CACHESIZE 4096
cl_file32_f::cl_file32_f(void)
:m_fp(NULL)
,m_state(0)
...
int cl_file32_f::open(const char *_Filename,int _Mode)
{
	if(m_fp)
		return -1;
...
bool cl_file32_f::is_open() const 
{
	return (m_fp!=NULL);
}
void cl_file32_f::close()
{
	if(m_fp)
	{
		flush();
...
int cl_file32_f::write(const char *buf,int len)
{
	if(!m_fp)
	{
		assert(0);
...
int cl_file32_f::read(char *buf,int len)
{
	if(!m_fp)
	{
		assert(0);
