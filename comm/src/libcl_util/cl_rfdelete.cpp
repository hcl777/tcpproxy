...
int cl_rfdelete_dir(const char* path)
{
	char dir[MAX_PATH];
	char dir_[MAX_PATH];
		...
		do{
			if(fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				if(0!=strcmp(fd.cFileName,".") && 0!=strcmp(fd.cFileName,".."))
				{
					...
					cl_rfdelete_dir(file_path);// 递归调用删除目录下的文件
				}
			}
			else
			{
...
int cl_rfdelete(const char* path)
{
	WIN32_FIND_DATAA fd;
	...
	if(hFind!=INVALID_HANDLE_VALUE)
	{
		if(fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
		{
			if(0!=strcmp(fd.cFileName,".") && 0!=strcmp(fd.cFileName,".."))
			{
				ret = cl_rfdelete_dir(path);// 递归调用删除目录下的文件
			}
		}
		else
		{
			...
			if(dir != NULL) 
			{
				while((0==readdir_r(dir,&u.d,&dir_ret)) && (NULL!=dir_ret))
				{
					...
					else 
					{
						...
						cl_rfdelete(file_path);// 递归调用删除目录下的文件
					}
				}
				closedir(dir);
				//printf("--delete-dir : %s \n",path);
