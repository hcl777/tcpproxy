...
{
    unsigned int index = 0,partlen = 0;
...
void cl_md5_final(CL_MD5_CTX *context,unsigned char digest[16])
{
    ...
    cl_md5_update(context,PADDING,padlen);
    cl_md5_update(context,bits,8);
...
{
	for(int i=0;i<16;++i)
		sprintf(outstr+2*i,"%02x",digest[i]);
}
char*  cl_md5_buffer(unsigned  char* buf,int size,char outstr[33])
{
	...
	CL_MD5_CTX ctx;
	cl_md5_init(&ctx);
	cl_md5_update(&ctx,buf,size);
	cl_md5_final(&ctx,digest);
	cl_md5_hx2str(digest,outstr);
	outstr[32] = '\0';
	return outstr;
}
