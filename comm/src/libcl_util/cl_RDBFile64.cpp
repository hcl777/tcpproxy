...
cl_RDBFile64::cl_RDBFile64(void)
: m_file_size(0)
, m_file_write_size(0)
, m_block_size(0)
, m_head_size(0)
...
int cl_RDBFile64::load_head()
{
	...
	if(1==ver)
	{
		...
		while(blocks>0)
		{
			...
			read_len = n * 8; //100块为800个字节，不到10K
...
int cl_RDBFile64::check_resize_logbs_ok(int blocks)
{
...
int cl_RDBFile64::open(const char* path,int mode)
{
...
int cl_RDBFile64::close()
{
	_file.close();
	reset();
	return 0;
}
ssize64_t cl_RDBFile64::seek(ssize64_t distance,int smode)
{
	if(!_file.is_open())
	{
		assert(0);
...
int cl_RDBFile64::write(char *buf,int len)
{
	if(!_file.is_open())
	{
		assert(0);
...
int cl_RDBFile64::read(char *buf,int len)
{
	if(!_file.is_open())
	{
		assert(0);
...
int cl_RDBFile64::flush()
{
	return _file.flush();// flush影响读写性能极大，视听馆极容易卡片。
}
ssize64_t cl_RDBFile64::tell()
{
	return m_pos;
}
int cl_RDBFile64::resize(ssize64_t size)
{
	if(!_file.is_open())
	{
		return -1;
	...
	if(size>0)
	{
		return cl_BFile64::resize(m_head_size + size);
	}
	return 0;
}
int cl_RDBFile64::get_checkinfo(const char* path, cl_RDBFile64::CheckInfo_t& inf)
{
	enum {OPEN_FAIL=-1,OK=0,READ_FAIL=1};
	cl_file64 file;
	char buf[1024];
	int endian = 0;
	int ver = 0;
	int blocks, n,read_len,index, bsize;

	if (0 != file.open(path, F64_READ))
		return OPEN_FAIL;
	inf.seekend_endpos = file.seek(0, SEEK_END);

	file.seek(0, SEEK_SET);
	if (48 != file.read(buf, 48))
		return READ_FAIL;
	memcpy((char*)&endian, buf + 40, 4);
	memcpy((char*)&ver, buf + 44, 4);
	//注意:在此判断endian有没有跟原来写入的序相反,若相反实际上是要交换的,暂时这里不支持那么快吧

	if (0 == strncmp(buf, RDB_STX, 40) && 1==ver)
	{
		//只支持版本1
		if (0 != file.read_n(buf, 28))
			return READ_FAIL;
		memcpy((char*)&inf.head_size, buf + 0, 4);
		memcpy((char*)&inf.record_file_size, buf + 4, 8);
		memcpy((char*)&inf.record_write_size, buf + 12, 8);
		memcpy((char*)&inf.block_num, buf + 20, 4);
		memcpy((char*)&inf.block_size, buf + 24, 4);

		blocks = inf.block_num;
		inf.calculate_write_size = 0;
		inf.calculate_file_size = 0;
		inf.calculate_endpos = 0;
		inf.max_index = -1;
		index = 0;
		while (blocks > 0)
		{
			n = blocks>100 ? 100 : blocks;
			read_len = n * 8; //100块为800个字节，不到10K
			if (read_len != file.read(buf, read_len))
				return READ_FAIL;
			for (int i = 0; i < n; ++i)
			{
				memcpy((char*)&index, buf + i * 8, 4);
				memcpy((char*)&bsize, buf + i * 8 + 4, 4);
				if (bsize != RDB_BLOCK_SIZE)
				{
					//如果bsize > block size也是正常的，重复写入的情况。
					//printf("*** index = %d, bsize = %d \n", index, bsize);
				}
				if (index > inf.max_index)
				{
					inf.max_index = index;
					inf.calculate_file_size = index * (size64_t)inf.block_size + bsize;
				}
				inf.calculate_write_size += bsize;
			}
			blocks -= n;
		}
		inf.last_index = index;
		if(inf.block_num>0)
			inf.calculate_endpos = (inf.block_num-1)*(size64_t)inf.block_size + bsize + inf.head_size;
		return 0;
	}
	return 2;
}
...
//int cl_RDBFile64::eif_zip_file(const char* path,int id)
//{
//	if(!_file.is_open())
//	{
//		return -1;
...
//	inf.file_size = cl_file64::get_file_size(path);
...
//	return 0;
//}
//int cl_RDBFile64::eif_get_zip_info(RDBEIFInfo_t& zi)
//{
//	if(!_file.is_open())
//	{
//		return -1;
...
//	return 0;
//}
//int cl_RDBFile64::eif_get_zip_infoi(const char* name,RDBEIFNode_t& node)
//{
//	if(!_file.is_open())
//	{
//		return -1;
...
//	return ret;
//}
//int cl_RDBFile64::eif_unzip_all_file(const char* dir)
//{
//	if(!_file.is_open())
//	{
//		return -1;
...
//******************************************************
cl_ERDBFile64::cl_ERDBFile64(void)
: m_pfile(NULL)
...
int cl_ERDBFile64::open(const char* path,int mode,int ftype/*=RDBF_AUTO*/)
{
	if(m_pfile)
	{
		assert(0);
	...
	cl_BFile64 *pfile = NULL;
	if(RDBF_BASE==ftype)
		pfile = new cl_BFile64();
	else if(RDBF_RDB==ftype)
		pfile = new cl_RDBFile64();
	else if(RDBF_RDBS==ftype)
	{
		//只能是只读
		if(F64_READ!=mode)
			return -1;
		pfile = new cl_RDBFile64Simple();
	}
...
ssize64_t cl_ERDBFile64::seek(ssize64_t distance,int smode)
{
	if(m_pfile)
...
int cl_ERDBFile64::write(char *buf,int len)
{
	if(m_pfile)
...
int cl_ERDBFile64::read(char *buf,int len)
{
	if(m_pfile)
...
int cl_ERDBFile64::read_n(char *buf, int len)
{
	if (!is_open())
		return -1;
	int n = 0;
...
size64_t cl_ERDBFile64::get_filesize(const char* path)
{
	cl_file64 file;
	if(0!=file.open(path,F64_READ))
		return 0;
...
int cl_ERDBFile64::get_filetype(const char* path)
{
	cl_file64 file;
	if(0!=file.open(path,F64_READ))
		return RDBF_UNKNOW;
...
int cl_ERDBFile64::check_size_ok(const char* path)
{
	enum {FAIL=-1,OK=0,MISSING,MORE,FILE_UNKNOW};
	cl_file64 file;
	size64_t real_size,read_size;
	int head_size, block_size, block_num,block_offset;
	char buf[128];
	int endian = 0;
	int ver = 0;
	...
	if (0 != file.open(path, F64_READ))
		return FAIL;
	real_size = file.seek(0, SEEK_END);
	...
	if (0 == strncmp(buf, RDB_STX, 40))
	{
		if (1 == ver)
		{
			...
			if (0 != file.read_n(buf, 28))
				return FILE_UNKNOW;
			memcpy((char*)&head_size, buf + 0, 4);
			memcpy((char*)&block_num, buf + 20, 4);
			memcpy((char*)&block_size, buf + 24, 4);
			if (block_num <= 0 || block_num * 8 + 76 > head_size)
				return FILE_UNKNOW;
			if (real_size < (size64_t)head_size)
				return MISSING;

			file.seek((block_num - 1) * 8, SEEK_CUR);
			if (0 != file.read_n(buf, 8))
				return FILE_UNKNOW;
			memcpy((char*)&block_offset, buf + 4, 4);
			read_size = (block_num - 1)*(size64_t)block_size + block_offset + head_size;
			if (real_size < read_size)
				return MISSING;
			else if (read_size == real_size)
				return OK;
			else
				return MORE;
		}
