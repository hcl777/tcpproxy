...
int cl_thread::activate(int n/*=1*/)
{
	assert(0==m_nThr);
	if(m_nThr>0 || n<=0)
		return 0;
...
int cl_thread::wait(DWORD milliseconds/*=INFINITE*/)
{
...
int cl_thread::work(int e)
{
	return 0;
}
DWORD WINAPI cl_thread::_work_T(LPVOID p)
{
...
int cl_thread::_icreate_state = 0;
cl_thread::cl_thread(void): 
m_handle(NULL)
, m_nThr(0)
{
