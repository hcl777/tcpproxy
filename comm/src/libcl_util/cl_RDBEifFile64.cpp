...
ssize64_t rdbeif_get_beg_pos_base(cl_file64& file,size64_t pos)
{
	if(pos<8)
		return pos;
	char buf[32];
	...
	{
		//DEBUGMSG("#: *** rdbeif_get_beg_pos_base::read(8) faild \n");
		return pos;
	}
	if(size+8>(size64_t)pos || size<20) //越界
	{
		//DEBUGMSG("#: *** rdbeif_get_beg_pos_base:: size wrong(pos=%lld,%lld) faild \n",pos,size);
		return pos;
	}
	file.seek(pos-8-size,SEEK_SET);
	if(20!=file.read(buf,20))
	{
		//DEBUGMSG("#: *** rdbeif_get_beg_pos_base::read(20) faild \n");
		return pos;
	}
	...
	{
		//DEBUGMSG("#: *** rdbeif_get_beg_pos_base::memcmp(EIF_STX_BEG) faild \n");
	}
	return pos;
}
ssize64_t rdbeif_get_beg_pos(const char* path)
{
	...
	//if(RDB_ENDIAN != endian)
	//{
	//	DEBUGMSG("*** RDB_ENDIAN wrong(%x, %x)\n",endian,RDB_ENDIAN);
	//}
	...
	{
		//DEBUGMSG("rdbeif_get_beg_pos::base file(size=%lld) \n",size);
	}
	return rdbeif_get_beg_pos_base(file,size);
}
size64_t rdbeif_get_mainfile_size(const char* cvipath)
{
	if(RDBF_BASE==cl_ERDBFile64::get_filetype(cvipath))
		return rdbeif_get_beg_pos(cvipath);
	else
		return cl_ERDBFile64::get_filesize(cvipath);
}
int rdbeif_read_node_info(cl_file64& file,ssize64_t bpos,rdbeif_node_t& inf)
{
	char buf[256];
	char buf_end[16];
	if(bpos!=file.seek(bpos,SEEK_SET))
	{
		//DEBUGMSG("#: *** seek(%lld) fail! \n",bpos);
		return -1;
	}
	if(EIF_HEAD_SIZE!=file.read(buf,EIF_HEAD_SIZE))
	{
		//DEBUGMSG("#: *** read(EIF_HEAD_SIZE) fail! \n");
		return -1;
	}
	if(0!=strncmp(buf,EIF_STX_BEG,8))
	{
		//DEBUGMSG("#: *** strncmp(EIF_STX_BEG) fail! \n");
		return -1;
	}
	int pos = 8;
	...
	if(8!=file.read((char*)&len,8))
	{
		//DEBUGMSG("#: *** read(8) fail! \n");
		return -1;
	}
	//len = *(size64_t*)buf_end;
	if(0!=strncmp(buf_end,EIF_STX_END,8) && len!=(inf.file_size+inf.head_size))
	{
		//DEBUGMSG("#: *** strncmp(EIF_STX_END) fail! \n");
		return -1;
	}
...
int rdbeif_write_node_info(cl_file64& file,ssize64_t bpos,rdbeif_node_t& inf)
{
	char buf[256];
	int pos = 0;
	pos = 0;
...
int rdbeif_copy_file(cl_file64& from,ssize64_t from_pos,cl_file64& to,ssize64_t to_pos,size64_t size)
{
...
rdbeif_handle_t* rdbeif_open(const char* cvipath,int mode)
{
	...
	h->file_type = cl_ERDBFile64::get_filetype(cvipath);
...
int rdbeif_clean(const char* cvipath)
{
	...
	size64_t size = cl_file64::get_file_size(cvipath);
	if(eif_beg_offset<size)
	{
		return cl_file64::resize(cvipath,eif_beg_offset);
	}
	return 0;
}
