...
void cl_aes::set_key(const u1byte in_key[], const u4byte key_len)
{   
	u4byte  i, t, u, v, w;
	if(!tab_gen)
		gen_tabs();
...
#define f_lround(bo, bi, k) \
    ...
    f_rl(bo, bi, 2, k);     \
f_rl(bo, bi, 3, k)
void cl_aes::encrypt(const u1byte in_blk[16], u1byte out_blk[16])
{   
	u4byte  b0[4], b1[4], *kp;
...
#define i_lround(bo, bi, k) \
    ...
    i_rl(bo, bi, 2, k);     \
i_rl(bo, bi, 3, k)
void cl_aes::decrypt(const u1byte in_blk[16], u1byte out_blk[16])
{
	u4byte  b0[4], b1[4], *kp;
...
void cl_aes::encrypt_n(u1byte iobuf[],int size)
{
...
void cl_aes::decrypt_n(u1byte iobuf[],int size)
{
	unsigned char outbuf[32];
	for(int i=0;i<size-16;i+=16)
	{
...
//随机生成key,一般为16B或者32B.
int cl_aes::rand_key(u1byte key[],int size)
{
