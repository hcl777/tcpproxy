...
{
	if(UAC_DISCONNECTED!=m_uac_state||0==ip||0==port||nattype>6||nattype<0)
	{
		//assert(0);
		return -1;
	}
...
int UAC_Socket::uac_setbandwidth(int size)
{
	if(m_uac_fd>=0)
		return ::uac_setbandwidth(m_uac_fd,size);
	return -1;
}
...
int UAC_Socket::uac_getsendspeed(UAC_sendspeed_t& si)
{
	if(m_uac_fd>=0)
	{
		si = ::uac_getsendspeed(m_uac_fd);
		return 0;
	}
	return -1;
}
int UAC_Socket::uac_getrecvspeed(UAC_recvspeed_t& si)
{
	if(m_uac_fd>=0)
	{
		si = ::uac_getrecvspeed(m_uac_fd);
		return 0;
	}
	return -1;
}
...
void UAC_Socket::uac_on_write()
{
	if(UAC_CONNECTING==m_uac_state)
	{
		...
		//检查是否是断开.
		if(::uac_is_connected(m_uac_fd))
			uac_on_connected();
		else
			uac_on_connect_broken();
	}
	else
	{
		if(!m_uac_blastsend)
