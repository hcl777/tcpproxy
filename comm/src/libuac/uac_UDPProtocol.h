...
#define UDPHEAD_UPDATE_TTLMS(buf,ttlms) *(uint16*)(buf+16)=SerialStream::htol16(ttlms)

//连接头9字节,UDP数据包命令1个字节
#define UDPHEAD_LENGTH 20
...
typedef struct tagUDPSAck
{
	...
	uint16		csp_num;
	uint32		csp_speedB;
	uint32		const_send_speedKB;		//指定对方以这个速度发送
...
public:
	UDPChannelHandler(int idx) : __idx(idx),m_fd(INVALID_SOCKET),m_mtu(1400){}
