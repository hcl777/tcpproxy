...
int sockpool::close_socket(UAC_SOCKET fd)
{
...
int sockpool::setbandwidth(UAC_SOCKET fd,int size)
{
	if(size>0 && size<20000) size=20000; //最少20K
	return m_socks[fd].ch->set_bandwidth(size);
}
int sockpool::setrecvbuf(UAC_SOCKET fd,int size)
{
	if(size<20000) size=20000; //最少20K
	return m_socks[fd].ch->set_recvbuf(size);
}
...
bool sockpool::is_connected(UAC_SOCKET fd)
{
	return (SOCK_CONNECTED==m_socks[fd].sock_state);
}
...
int sockpool::recv(UAC_SOCKET fd,char* buf,int len)
{
	...
	else
	{
		...
		//限接收速度,如果延迟接收，则睡8毫秒
		int max_recvsize = statisticsSngl::instance()->get_max_recvsize();
		if(len > max_recvsize)
			len = max_recvsize; 
		if(max_recvsize==0)
			Sleep(1);
		...
		else
		{
			...
			statisticsSngl::instance()->m_recvspeed.add(recvsize);
		}
