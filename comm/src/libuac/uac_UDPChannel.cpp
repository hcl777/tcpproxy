...
void UDPChannel::on_connected()
{
	...
	assert(m_mtu>0);
	if(g_uac_conf.packet_size>0)
		m_mtu = g_uac_conf.packet_size;
	m_mtu -= (60+UDPHEAD_LENGTH); //去掉IP头和UDP头长度28,减多一些,遇到过减45个都发不了的情况.
	m_send_win.spctrl.mtu = m_mtu;

	m_state = CONNECTED;
	TimerSngl::instance()->register_timer(this,TIMER_ACK,g_uac_conf.udp_nak_timer_msec);
	TimerSngl::instance()->register_timer(this,TIMER_SECOND,1000);
...
void UDPChannel::on_disconnected()
{
	...
	reset();
	fire(ChannelListener::Disconnected(),static_cast<Channel*>(this));
}
...
int UDPChannel::set_bandwidth(int size)
{
	m_send_win.spctrl.csp.bandwidth = size;
	if(size>0)
	{
		m_send_win.spctrl.csp.speedB = size;
		//UACLOG("#SET bandwidth = %d byte \n",size);
	}
	return 0;
}
int UDPChannel::set_recvbuf(int size)
{
	m_recv_win.max_recv_win_num = size/m_mtu;
	if(m_recv_win.max_recv_win_num<20) m_recv_win.max_recv_win_num = 20;
	return 0;
}
...
int UDPChannel::send_to(char *buf,int len,ULONGLONG utick)
{
	if(buf[UDPS_CONN_HEAD_LENGTH] == UDPS_CHANNEL_CMD_DATA)
	{
		...
		UDPHEAD_UPDATE_TTLMS(buf,(uint16)(m_send_win.ttl/1000));
		m_send_win.spctrl.csp.on_send(utick,len);
		m_send_win.ssi.last_sendB += len;
		m_send_win.ssi.last_send_num++;
	}
...
int UDPChannel::handle_send(bool roolcall/*=false*/)
{
	...
	{
		//UACLOG("*** send list empty *** \n");
		return 0;
	}
	
	unsigned int n = 0,sn=0,rn=0;
	ULONGLONG utick = uac_GetUTickCount();
...
//		max_count=1000;
	...
	for(SendIter it=m_send_list.begin();it!=m_send_list.end()&&n<max_count;++it)
	{
		...
		else if(pack->send_count<g_uac_conf.max_resend_num)
		{
			bsend = false;
			...
			else
			{
				...
				{
					//预计可能丢包
					timeout_us = m_send_win.resend_timeout_mod*m_send_win.ttl+80000;
					if(timeout_us>5000000) timeout_us = 5000000;
					if(timeout_us<300000) timeout_us = 300000; //最少隔300ms
					if((pack->last_send_tick + timeout_us) < utick)
					{
				...
				{
					//超时重发
					timeout_us = m_send_win.resend_timeout_mod*m_send_win.ttl+100000;
					if(timeout_us>5000000) timeout_us = 5000000;
					if(timeout_us<300000) timeout_us = 300000; //最少隔300ms
					if((pack->last_send_tick + timeout_us) < utick)
					{
...
int UDPChannel::handle_recv_data(PTLStream& ps,memblock* b)
{
	...
	ULONGLONG curr_utick = uac_GetUTickCount();
	...
	m_recv_win.sri.last_recvB += b->datasize;
	m_recv_win.sri.last_recv_num++;
	m_recv_win.other_ttlms = _tmp_recv_data.ttlms;
	if(m_recv_win.other_maxttlms < m_recv_win.other_ttlms)
		m_recv_win.other_maxttlms = m_recv_win.other_ttlms ;
...
int UDPChannel::handle_recv_ack(PTLStream& ps)
{
	...
	m_send_win.spctrl.on_recv_ack(tick,utick);
	...
	{
		m_send_win.other_rerecv_num = ack.rerecv_num;
		if(ack.ack_sequence>m_send_win.ack_sequence)
		{
...
void UDPChannel::send_ack(DWORD tick,ULONGLONG utick)
{
...

	{
		...
		m_recv_win.ack.const_send_speedKB = 0;
		m_recv_win.ack.const_send_lose_rate = 0;
...
void UDPChannel::on_timer_ack()
{
	ULONGLONG utick = uac_GetUTickCount();
	unsigned int tick = GetTickCount();
	
	unsigned time_out = g_uac_conf.udp_nak_timer_msec*(m_recv_win.resend_ack_win_low_line_count+1)-5;
	if(_timer_after(tick,m_recv_win.last_ack_tick+time_out) && m_recv_win.resend_ack_win_low_line_count<20)
		send_ack(tick,utick);

}
void UDPChannel::on_timer_second()
{
	//m_send_win.spctrl.on_second();

	//send
	m_send_win.ssi.si.ttlms = m_send_win.maxttl/1000;
	m_send_win.maxttl = m_send_win.ttl;
	m_send_win.ssi.si.lost_rate = m_send_win.spctrl.csp.get_lost_rate(3);
	m_send_win.ssi.si.seq_range = m_send_win.real_send_num - m_send_win.win_low_line;
	m_send_win.ssi.last_rerecv_num = m_send_win.other_rerecv_num - m_send_win.ssi.last_rerecv_num;
	m_send_win.ssi.on_second();
	m_send_win.ssi.last_rerecv_num = m_send_win.other_rerecv_num;
	if(m_send_win.ssi.si.rerecv_rate>5)
	{
		m_send_win.resend_timeout_mod += 2;
	}
	else
	{
		m_send_win.resend_timeout_mod = 3;
	}

	//recv
	m_recv_win.sri.si.ttlms = m_recv_win.other_maxttlms;
	m_recv_win.other_maxttlms = m_recv_win.other_ttlms ;
	m_recv_win.sri.si.seq_range = m_recv_win.max_sequence_num - m_recv_win.win_low_line;
	m_recv_win.sri.last_rerecv_num = m_recv_win.rerecv_num - m_recv_win.sri.last_rerecv_num;
	m_recv_win.sri.on_second();
	m_recv_win.sri.last_rerecv_num = m_recv_win.rerecv_num;
