...
int UDPAcceptor::open(unsigned short port,const char* stunsvr,unsigned short stun_port,UDPChannelFactory* chf)
{
	...
	if(0!=open_sock(port,NULL))
		return -1;
	if(stunsvr && strlen(stunsvr) && stun_port>0)
	{
...
int UDPAcceptor::handle_input()
{
	...
	//utick = (uac_GetUTickCount()-utick);
	//if(utick>50000 && n>0)
	//	UACLOG("# -%d - n=%d --> %d \n",(int)utick/1000,n ,(int)(utick/n));
	return 0;
}
...
{
	UACLOG("$UAC_nat = %d \n",nat_type);
	g_uac_conf.nattype = nat_type;
	if(g_uac_conf.callback_onnatok)
		g_uac_conf.callback_onnatok(nat_type);
}
void UDPAcceptor::on(UDPStunClientListener::BindingAddrChanged,unsigned int ip,unsigned short port)
{
	UACLOG("$UAC_ipport = %s:%d \n",Util::ip_htoa(ip),port);
	if(g_uac_conf.callback_onipportchanged)
		g_uac_conf.callback_onipportchanged(ip,port);
}
