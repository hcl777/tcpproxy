...
int UDPConnector::handle_cmd_ok(UDPSConnHeader_t& head,char* buf,int size,sockaddr_in& addr,PTLStream& ps)
{
	...
	if(UDP_CONNECTING == m_udps[i]->state || UDP_ACCEPTING == m_udps[i]->state)
	{
		...
		else if(UDP_ACCEPTING == m_udps[i]->state)
		{
...
void UDPConnector::timer_resend()
{
	...
	for(int i=0;i<=m_max_i;++i)
	{
		...
		if(UDP_ACCEPTING == m_udps[i]->state || UDP_CONNECTING == m_udps[i]->state)
		{
		...
		//此连接15秒内没有收过数据即超时断开
		if((m_udps[i]->last_recv_tick+g_uac_conf.udp_conn_timeout_msec) < m_tick)
		{
			//UACLOG("#[udp un-active timeout!(%d sec)] \n",g_uac_conf.udp_conn_timeout_msec/1000);
			m_udps[i]->handle->handle_disconnected();
			continue;
		}
		...
		if(m_udps[i]->state == UDP_CONNECTED || m_udps[i]->state == UDP_ACCEPTED)
		{
			...
			if(m_udps[i]->last_send_tick+g_uac_conf.udp_keeplive_timer_msec < m_tick)
			{
				//keeplive
