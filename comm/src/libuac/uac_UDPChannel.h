...
typedef struct tag_SecondSendInfo
{
	...
	void on_second()
	{
		si.speedB = last_sendB;
		if(last_send_num)
			si.rerecv_rate = last_rerecv_num*100/last_send_num;
		else
			si.rerecv_rate = 0;
...
public:
	...
	virtual UAC_sendspeed_t& get_sendspeed(){return m_send_win.ssi.si;}
