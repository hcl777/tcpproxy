...
int main(int argc,char** argv)
{
	...
	int i = 0;
	if(-1!=(i=cl_util::string_array_find(argc,argv,"--update")))
	{
		if(i+3<argc)
		{
			//--update [path ver url]
	...
	if(-1!=(i=cl_util::string_array_find(argc,argv,"--reload")))
	{
		if(i+2<argc)
		{
...
int update(const string& path,const string& ver,const string& verurl)
{
	...
	if(0!=cl_httpc::http_get(verurl,body,1024))
	{
		return -1;
	}
	strbody = body;
	...
	if(0!=cl_httpc::download_file(new_url,path))
		return -1;
...
int reload(const string& newfile,const string& path,int argc,char** argv)
{
	...
	string bak = path;
	bak += ".bak";
	if(!cl_util::file_exist(newfile))
		return -1;
	cl_util::file_delete(bak);
	if(cl_util::file_exist(path))
	{
		//将文件移到bak
		int n = 0;
		while(0!=cl_util::file_rename(path,bak))
		{
			if(++n>20) break;
			Sleep(100);
		}
	}
	if(0!=cl_util::file_rename(newfile,path))
		return -1;
