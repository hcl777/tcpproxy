...
string HttpResponseHeader::DateToStr(const SYSTEMTIME& st)
{
  ...
  //DEBUGMSG("#HttpResponseHeader::DateToStr() date: week = %d %d-%d-%d %d:%d:%d GMT\n", (int)st.wDayOfWeek, 
  //             (int)st.wDay, (int)st.wMonth, (int)st.wYear, (int)st.wHour, (int)st.wMinute, (int)st.wSecond);
  ...
  sprintf(buf,"%s, %02d %s %04d %02d:%02d:%02d GMT", sDay[st.wDayOfWeek], 
               (int)st.wDay, sMonth[st.wMonth], (int)st.wYear, (int)st.wHour, (int)st.wMinute, (int)st.wSecond);
  return buf;
}
...
BOOL HttpResponseHeader::Send(SOCKET socket,int timeoutms)
{
  ...
  return 0==cl_net::sock_select_send_n((int)socket,m_sHeader.c_str(), (int)m_sHeader.length(),timeoutms);
